...
function writeTypingBossEvent(subscriber, session) {
  const event = `event: session\ndata: ${JSON.stringify(
    getTypingBossSnapshot(session, {
      includeChallengesFor: subscriber.auth?.role === "player" ? subscriber.auth.actorCode : "",
    })
  )}\n\n`;
...
server.on("error", (error) => {
  if (error.code === "EADDRINUSE") {
    console.error(
      `Padlet backend could not start because port ${PORT} is already in use.`
    );
    console.error(`Stop the process using ${PORT}, or set PADLET_PORT.`);
    process.exit(1);
  }

  console.error("Padlet backend failed to start.", error);
  process.exit(1);
});

server.listen(PORT, () => {
  console.log(`Padlet backend listening on http://localhost:${PORT}`);
});

function shutdown(signal) {
  console.log(`Padlet backend received ${signal}; closing active sessions.`);
