...
function loadPersist(): PersistShape {
  try {
    ...
    if (!parsed || typeof parsed.currentZ !== "number" || !Array.isArray(parsed.list)) {
      return { currentZ: 1, list: [] };
    }

    const trimmed = parsed.list.length > MAX_ENTITIES ? parsed.list.slice(-MAX_ENTITIES) : parsed.list;
...
function savePersist(p: PersistShape) {
  try {
    const out: StorageShape = {
      currentZ: p.currentZ,
      list: p.list.length > MAX_ENTITIES ? serializeEntities(p.list.slice(-MAX_ENTITIES)) : serializeEntities(p.list),
    };
...
export default function Demo() {
  // === NEW: Auto‑run switch state ===
  const [autoRun, setAutoRun] = useState(false);

  const [show, setShow] = useState(false);
  const [toast, setToast] = useState<{ msg: string; type: "success" | "error" } | null>(null);

  const [{ currentZ, list }, setPersist] = useState<PersistShape>(() => loadPersist());
  const [bunnyKick, setBunnyKick] = useState(0);

  useEffect(() => { savePersist({ currentZ, list }); }, [currentZ, list]);
  ...
  // === NEW: when Auto‑run flips ON, trigger a headless run once ===
  useEffect(() => {
    if (autoRun) setShow(true);
  }, [autoRun]);
  ...
  function spawnGrasses(count = 5) {
    setPersist((prev) => {
      ...
      for (let i = 0; i < count; i++) {
        ...
        const g = new GrassEntity({ id: Date.now() + Math.random(), dead: false, z: currentZ, liftPx: lift });
        list = [...list, g];
      }
  ...
  const addAliveBunny = () => {
    ...
    const bun = new BunnyEntity({ id: Date.now() + Math.random(), dead: false, z: newZ, liftPx: lift });
  ...
  const addWolf = () => {
    const newZ = (currentZ ?? 1) + 1;
    const lift = rowLiftFromZ(newZ);
    const w = new WolfEntity({ id: Date.now() + Math.random(), dead: false, z: newZ, liftPx: lift });
  ...
  function replaceDeadWolfWithBear(): boolean {
    const deadIdx = list.map((e, i) => (e.kind === "wolf" && e.dead ? i : -1)).filter((i) => i >= 0);
    if (!deadIdx.length) return false;
    const pick = deadIdx[Math.floor(Math.random() * deadIdx.length)];
    const w = list[pick];
    const bear = new BearEntity({ id: Date.now() + Math.random(), dead: false, z: w.z, liftPx: w.liftPx });
    const updated = [...list];
  ...
  function replaceDeadBunnyWithFox(): boolean {
    const deadIdx = list.map((e, i) => (e.kind === "bunny" && e.dead ? i : -1)).filter((i) => i >= 0);
    if (!deadIdx.length) return false;
    const pick = deadIdx[Math.floor(Math.random() * deadIdx.length)];
    const b = list[pick];
    const fox = new FoxEntity({ id: Date.now() + Math.random(), dead: false, z: b.z, liftPx: b.liftPx });
    const updated = [...list];
  ...
  function replaceDeadFoxWithWolf(): boolean {
    const deadIdx = list.map((e, i) => (e.kind === "fox" && e.dead ? i : -1)).filter((i) => i >= 0);
    if (!deadIdx.length) return false;
    const pick = deadIdx[Math.floor(Math.random() * deadIdx.length)];
    const f = list[pick];
    const wolf = new WolfEntity({ id: Date.now() + Math.random(), dead: false, z: f.z, liftPx: f.liftPx });
    const updated = [...list];
  ...
  /** Random plan chooser used on run failure */
  function runRandomFailurePlan() {
    ...
    const plans: Array<{ canRun: () => boolean; run: () => boolean }> = [
      { canRun: () => hasDead.bunny, run: () => replaceDeadBunnyWithFox() },
      { canRun: () => hasDead.fox, run: () => replaceDeadFoxWithWolf() },
      { canRun: () => hasDead.wolf, run: () => replaceDeadWolfWithBear() },
      { canRun: () => hasLive, run: () => killRandomEntity() },
    ];
    ...
    for (let i = 0; i < MAX_ROLLS; i++) {
      ...
      if (plan.canRun() && plan.run()) return;
    }
    if (hasLive) killRandomEntity();
  }
    ...
    <>
      ...
      <div className="serene-shell">
        ...
        <main className="card">
          <header className="header" style={{ position: "relative" }}>
            <h1>Take a breath.</h1>
            <p className="subtitle">This page will always be calm, no matter what happens.</p>

            {/* === NEW: Top-right Auto‑run switch === */}
            <label
              style={{ position: "absolute", top: 12, right: 16, display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: "var(--muted)", userSelect: "none", cursor: "pointer" }}
              title="When enabled, runs without showing the modal."
            >
              Auto‑run
              <input type="checkbox" checked={autoRun} onChange={(e) => setAutoRun(e.target.checked)} style={{ width: 34, height: 18, cursor: "pointer" }} />
            </label>
          </header>

          {/* Hide Begin button when Auto‑run is ON */}
          {!autoRun && (
          ...
          {DEBUG && (
            <div style={{ display: "grid", gap: 8, marginTop: 12 }}>
              <button className="primary" style={{ background: "linear-gradient(180deg, #888, #666)" }} onClick={addAliveBunny} title="Spawn Bunny (z-index + log lift)">
                + Add Bunny (debug)
              </button>
              <button className="primary" style={{ background: "linear-gradient(180deg, #4e8c55, #2c6e36)" }} onClick={() => spawnGrasses(5)} title="Spawn Grass (z-index + log lift)">
                + Add Grass (debug)
              </button>
              <button className="primary" style={{ background: "linear-gradient(180deg, #555, #333)" }} onClick={addWolf} title="Spawn Wolf">
                + Add Wolf (debug)
              </button>
              <button className="primary" style={{ background: "linear-gradient(180deg, #9d2b2b, #7b1a1a)" }} onClick={killRandomEntity} title="Randomly mark one alive entity as dead">
                ☠ Kill Random (debug)
              </button>
              <button className="primary" style={{ background: "linear-gradient(180deg, #3a7c3a, #2e6b2e)" }} onClick={spawnRandomGenerator} title="Randomly spawns Bunny (1) or Grass (25)">
      ...
      <div className="entity-stage">
        {list.map((e) => {
          switch (e.kind) {
            case "bunny":
              return <Bunny key={e.id} active={!e.dead} dead={e.dead} kick={bunnyKick} z={e.z} liftPx={e.liftPx} />;
            case "grass":
              return <Grass key={e.id} dead={e.dead} z={e.z} liftPx={e.liftPx} xvw={xvwForId(e.id)} />;
            case "fox":
              return (
                <Fox
                  key={e.id}
            ...
            case "wolf":
              return <Wolf key={e.id} active={!e.dead} dead={e.dead} z={e.z} liftPx={e.liftPx} />;
            case "bear":
              return <Bear key={e.id} active={!e.dead} dead={e.dead} z={e.z} liftPx={e.liftPx} />;
            default:
              return null;
          }
        })}
      ...
      {/* Mount the modal and pass `headless` when Auto‑run is enabled */}
      {show && (
        <FetchProgressModal
          title="Processing…"
          simulate
          ...
          onSuccess={() => {
            ...
            setBunnyKick((k) => k + 1);
            if (autoRun) setTimeout(() => setShow(true), 250);
          }}
          onError={() => {
            setShow(false);
...
function SereneStyles() {
  return (
    <style>{`
      ...
      .entity-stage{ position: fixed; inset: 0; pointer-events: none; z-index: 2; }
      ...
      .toast.error{ background: linear-gradient(180deg, #b23c17, #8d2c12); }

      /* Ensure UI and creatures stay above background orbs */
      .card{ z-index: 3; }
