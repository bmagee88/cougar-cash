...
const ROWS: KeySlot[][] = [
  ...
  [
    ...
    { label: ";", widthU: 1 },
    { label: "'", widthU: 1 },
    { label: "Enter", widthU: 2.25 },
  ],
  [
    { label: "Shift", widthU: 2.25 },
    { label: "Z", widthU: 1 },
    { label: "X", widthU: 1 },
    { label: "C", widthU: 1 },
    { label: "V", widthU: 1 },
    ...
    { label: ".", widthU: 1 },
    { label: "/", widthU: 1 },
    { label: "Shift", widthU: 2.75 },
  ],
  [
    { label: "Ctrl", widthU: 1.25 },
    { label: "Meta", widthU: 1.25 },
    { label: "Alt", widthU: 1.25 },
    { label: "Space", widthU: 6.25 },
    { label: "Alt", widthU: 1.25 },
    { label: "Meta", widthU: 1.25 },
    { label: "Menu", widthU: 1.25 },
    { label: "Ctrl", widthU: 1.25 },
  ],
];
...
function labelsForDifficulty(d: Difficulty): string[] {
  ...
  const pushWithCount = (label: string) => {
    ...
    for (let i = 0; i < count; i++)
      list.push(i ? `${label} (${i + 1})` : label); // visually distinguish duplicates in pool
  };
...
type SlotStateMap = Record<
  string,
  { keyLabel: string | null; spotlight: boolean; stars: boolean }
>;
...
const EXPECTED_BY_SLOT: Record<string, string> = ALL_SLOTS.reduce((acc, s) => {
  acc[s.slotId] = s.expected;
  return acc;
}, {} as Record<string, string>);
const WIDTHU_BY_LABEL: Record<string, number[]> = ROWS.flat().reduce(
  (acc, k) => {
...
function randomGoal(totalKeys: number): Goal {
  const choices: Goal[] = [
    ...
    {
      kind: "avg",
      targetMs: 1400 + Math.floor(Math.random() * 800),
      reward: 150,
    }, // <1.4–2.2s
    {
      kind: "placed",
      target: Math.min(totalKeys, 4 + Math.floor(Math.random() * 6)),
      reward: 120,
    }, // 4..9
  ];
...
function useSfx() {
  ...
  return {
    correct: () => {
      ...
      setTimeout(() => tone(1244.5, 120, "triangle", 0.07), 120);
    },
    wrong: () => {
      tone(200, 200, "sawtooth", 0.06);
    },
    goal: () => {
      tone(932, 100, "square", 0.07);
      setTimeout(() => tone(1175, 100, "square", 0.07), 100);
    },
    finish: () => {
      tone(659, 140, "triangle", 0.09);
      setTimeout(() => tone(784, 140, "triangle", 0.09), 150);
      setTimeout(() => tone(988, 200, "triangle", 0.09), 300);
    },
    prime: ensureCtx,
  };
}
...
export default function App() {
  ...
  useEffect(
    ...
    [showHints, waitSeconds, difficulty, roundSeconds, themeMode]
  );

  /* Game state */
  const allowedLabels = useMemo(
    () => labelsForDifficulty(difficulty),
    [difficulty]
  );
  const [keysState, setKeysState] = useState<KeyStateMap>({});
  const [slotState, setSlotState] = useState<SlotStateMap>(() => {
    const initSlots: SlotStateMap = {};
    ALL_SLOTS.forEach(
      ({ slotId }) =>
        (initSlots[slotId] = { keyLabel: null, spotlight: false, stars: false })
    );
    return initSlots;
  });
  ...
  const [goal, setGoal] = useState<Goal>(() =>
    randomGoal(allowedLabels.length)
  );
  ...
  const setSlotRef = useCallback(
    (slotId: string) => (el: HTMLDivElement | null) => {
      slotRefs.current[slotId] = el;
    },
    []
  );
  const findHoveredSlot = useCallback(
  ...
  const finishDrop = (labelInPool: string, slotId: string | null) => {
    ...
    if (
      !slotId ||
      slotState[slotId].keyLabel !== null ||
      !keysState[labelInPool] ||
      keysState[labelInPool].status === "correct"
    )
      return;

    setKeysState((prev) => ({
      ...prev,
      [labelInPool]: { slot: slotId, status: "pending" },
    }));
    setSlotState((prev) => ({
      ...prev,
      [slotId]: {
        ...prev[slotId],
        keyLabel: labelInPool,
        spotlight: true,
        stars: false,
      },
    }));
    ...
    window.setTimeout(() => {
      setSlotState((currSlots) => {
        ...
        if (correct) {
          ...
          const deltaMs = lastCorrectRef.current
            ? now - lastCorrectRef.current
            : 3000;
          ...
          setKeysState((kPrev) => ({
            ...kPrev,
            [labelInPool]: { slot: slotId, status: "correct" },
          }));
          ...
          if (partnerTop && placedBase === expected) {
            ...
            if (topIdx)
              setKeysState((kPrev) => ({
                ...kPrev,
                [topIdx]: { slot: "__alias__", status: "aliased" } as any,
              }));
          }

          return {
            ...currSlots,
            [slotId]: { ...currSlots[slotId], spotlight: false, stars: true },
          };
        } else {
          sfx.wrong();
          setScore((s) => s - 25);
          ...
          setKeysState((kPrev) => ({
            ...kPrev,
            [labelInPool]: { slot: null, status: "idle" },
          }));
          const cleared: SlotStateMap = {
            ...currSlots,
            [slotId]: {
              ...currSlots[slotId],
              keyLabel: null,
          ...
          window.setTimeout(
            () => setReturningAnim((r) => ({ ...r, [labelInPool]: false })),
            600
          );
          return cleared;
        }
  ...
  const poolKeys = useMemo(() => {
    const inPool = Object.keys(keysState).filter(
      (k) => keysState[k].slot === null && keysState[k].status === "idle"
    );
    const orderIdx: Record<string, number> = {};
    poolOrder.forEach((k, i) => (orderIdx[k] = i));
  ...
  const GoalsBar = () => {
    const chip = (() => {
      switch (goal.kind) {
        case "streak":
          return `GOAL: Streak ${goal.target} (+${goal.reward})`;
        case "avg":
          return `GOAL: Avg < ${(goal.targetMs / 1000).toFixed(1)}s (+${
            goal.reward
          })`;
        case "placed":
          return `GOAL: Place ${goal.target} (+${goal.reward})`;
      }
    })();
    return (
      <Stack
      ...
      >
        ...
        <Chip
          variant="outlined"
          label={`Streak: ${streak} (Best: ${bestStreak})`}
        />
        ...
        {showHints && (base || top) && (
          ...
          >
            {top && (
              <Typography
                sx={{
                  fontSize: `${Math.max(10, unitPx * 0.24)}px`,
                  opacity: 0.55,
                  fontWeight: 700,
                  lineHeight: 1,
                  textAlign: "left",
            ...
            <Typography
              sx={{
                fontSize: `${Math.max(11, unitPx * 0.28)}px`,
                opacity: 0.7,
                fontWeight: 800,
                lineHeight: 1,
                textAlign: "right",
  ...
  const renderKeyboard = () => (
    ...
    >
      ...
      {ROWS.map((row, rIdx) => (
        <Box
          key={rIdx}
          sx={{ display: "flex", flexDirection: "row", gap: `${KEY_GAP_PX}px` }}
        >
          {row.map((k, cIdx) => {
            ...
            const keyIsCorrect =
              keyInSlot && keysState[keyInSlot]?.status === "correct";
              ...
              <Box
                ...
                sx={{
                  ...
                  boxShadow: isHovered
                    ...
                    : "inset 0 2px 4px rgba(2,6,23,0.10)",
                }}
              >
                {showHints && (base || top) && (
                  <Box
      ...
      {poolKeys.map((label) => {
        ...
        return (
          <Box
            ...
            title={
              BASE_FOR_SHIFT[plain]
                ? `${plain} (shift of ${BASE_FOR_SHIFT[plain]})`
                : plain
            }
          >
            {label}
        ...
        <Box
          sx={{
            ...
            boxShadow: isLight
              ...
              : "0 0 0 6px rgba(253,230,138,0.12)",
            zIndex: 5,
          }}
        >
          {draggingKey}
  ...
  const renderLeaderboard = () => (
    <Paper
      ...
      sx={{
        ...
        border: `4px solid ${colors.keyboardBorder}`,
        color: colors.text,
    ...
    >
      ...
      <List dense sx={{ overflowY: "auto", maxHeight: "calc(100% - 40px)" }}>
        {leaderboard.length === 0 && (
          <ListItem>
            <ListItemText primary="No scores yet. Be the first!" />
          </ListItem>
        )}
        {leaderboard.map((e, i) => (
          <React.Fragment key={i}>
            <ListItem>
              <ListItemText
                ...
                primary={`${i + 1}. ${e.name || "—"} — ${e.score}`}
                secondary={new Date(e.when).toLocaleDateString()}
              />
            </ListItem>
    ...
    >
      ...
      >
        ...
        <FormControlLabel
          ...
          label="Show correct letters"
        />

        <Box sx={{ mt: 2 }}>
          <Typography sx={{ fontSize: "0.95rem", mb: 0.5, fontWeight: 700 }}>
            Answer wait time: {waitSeconds}s
          </Typography>
          <Slider
            min={1}
            max={5}
            step={1}
            value={waitSeconds}
            onChange={(_, v) => setWaitSeconds(v as number)}
            valueLabelDisplay="auto"
          />
        </Box>

        <Box sx={{ mt: 2 }}>
          <Typography sx={{ fontSize: "0.95rem", mb: 0.75, fontWeight: 700 }}>
            Difficulty (1–5)
          </Typography>
          <ToggleButtonGroup
            exclusive
            value={difficulty}
            onChange={(_, v) => v && setDifficulty(v as Difficulty)}
            size="small"
          >
          ...
          <Typography sx={{ mt: 0.75, opacity: 0.7, fontSize: "0.9rem" }}>
            1: letters · 2: +numbers · 3: +punctuation · 4: +other keys · 5:
            +shift symbols
          </Typography>
        </Box>
        ...
        <Box sx={{ mt: 2 }}>
          ...
          <Slider
            ...
            step={5}
            value={roundSeconds}
            onChange={(_, v) => setRoundSeconds(v as number)}
            valueLabelDisplay="auto"
          />
      ...
      <AppBar position="static" color="primary">
        <Toolbar>
          ...
          <Stack direction="row" spacing={1}>
            <Button color="inherit" onClick={startRound}>
              {roundActive ? "Restart" : "Start"}
            </Button>
            <Button
              color="inherit"
              onClick={() => {
